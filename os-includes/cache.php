...
class OsimoCache{
	...
	function OsimoCache($prefix=false,$cache_addr='localhost',$cache_port=11211,$cache_time=300){
		if($prefix){
			$this->prefix = $prefix;
		}
		else{
	...
	public function memcacheInfo(){
		...
		foreach($statuses as $host=>$status){
        	...
        	echo "<tr><td>Server uptime:</td><td>".$status ["uptime"]." seconds ($uptime_hour hours)</td></tr>";
