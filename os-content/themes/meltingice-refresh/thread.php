...
if($osimo->arePosts($posts))
{
	...
	<?php
		...
		{
			echo " &bull; <a href=\"#\" onclick=\"deleteThread($threadID)\">Delete Thread</a>"; 
			if($osimo->isThreadSticky($threadID)){ echo " &bull; <a href=\"#\" onclick=\"stickyThread($threadID, false)\">Unsticky Thread</a>"; }
			else{ echo " &bull; <a href=\"#\" onclick=\"stickyThread($threadID,true)\">Sticky Thread</a>"; }
			if($osimo->isThreadLocked($threadID)){ echo " &bull; <img src=\"".THEMEPATH."img/icons/lock_edit.png\" alt=\"lock\" />&nbsp;<a href=\"#\" onclick=\"lockThread($threadID, false)\">Unlock Thread</a>"; }
			else{ echo " &bull; <img src=\"".THEMEPATH."img/icons/lock_edit.png\" alt=\"lock\" />&nbsp;<a href=\"#\" onclick=\"lockThread($threadID,true)\">Lock Thread</a>"; }
			?>
			&bull; Move Thread: <? $osimo->getForumDropdown(); ?>&nbsp;<input type="button" value="Go" onclick="moveThisThread(<?=$threadID?>)" />
			<?
		}
		else
		{
			if($osimo->isThreadLocked($threadID)){ echo " &bull; <span style=\"color: #c4c4c4\">Thread Locked</span>"; }
		} /* End administrative options */

		/* Pagination */
		echo "<span id=\"thread-nav\"> &bull; Page: <span id=\"osimo_pagination\">";
		echo $osimo->getPresetPagination('thread',$threadID,$page);
		echo "</span>&nbsp;</span>";	
		/* End pagination */	
		?>
	</div>
